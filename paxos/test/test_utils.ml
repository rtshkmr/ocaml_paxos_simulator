...
(** Creates a headless simulator capped at [max_ticks]. [allow_step] is NOT used
    here -- we drive the loop ourselves. Defaults to 30 ticks, which is enough
    for the basic happy-path scenario. *)
...
let run_to_completion sim =
  let rec loop () =
    if Sim.is_runnable sim then (
      Sim.step sim;
      loop ())
  in
  loop ()

(** Builds a {!Sim.N.spec} for a single node. [node_id] is 0 by default because
    the simulator will overwrite it with the next counter value when hydrating.
*)
...
(** Seeds [n] nodes named ["node_0"] .. ["node_{n-1}"] into [sim]. All nodes
    share the same [cluster_size] because we're sticking to static node clusters
    without needing to handle presence checking.*)
...
(** Schedules a proposal event at logical time [at]. [alias] is the proposer
    node's alias, [value] is the string value to propose, and [seq] is the
    proposal sequence number (defaults to 1). *)
...
(** Schedules a control event that moves [alias] to partition [dest] at logical
    time [at]. [dest] must be a partition id (integer ≥ 1). *)
...
(** Schedules a node reactivation ("revive") at logical time [at]. This makes a
    node inactive. *)
...
(** Schedules a node deactivation ("crash") at logical time [at]. This makes a
    node inactive. *)
...
(** Walk a role_state Sexp.t and find the value of a named field. e.g.
    [find_field "proposer" sexp] returns the sexp bound to that key. *)
let find_field name (sexp : Sexp.t) =
  match sexp with
  | List fields ->
      List.find_map fields ~f:(function
        | List [ Atom k; v ] when String.equal k name -> Some v
        | _ -> None)
  | _ -> None
...
let is_learned_state role_sexp =
  ...
  | List [ Atom "Learned"; List (_ :: _) ] -> true (* non-empty list *)
  | _ -> false
...
(** Extract the string value from a Decided sexp. Input shape: (Decided "some
    value") *)
let decided_value_of (proposer_sexp : Sexp.t) : string option =
  match proposer_sexp with
  | List [ Atom "Decided"; Atom v ] -> Some v
  | _ -> None
