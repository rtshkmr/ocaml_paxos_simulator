...
(** Helps us create [NS.V.t] (expected to just be a value-string) using the sexps workaround.
    [NS.V.t] is abstract from outside the functor, even though we know it's a [ string ] underneath, OCaml won't unify the two.
    The way to get a value of type [ NS.V.t ] without touching [ spec ] is via [ sexp ], since [t] derives it.
*)
...
(** Builds a role_state whose proposer is in WaitingForPromises with the
    given [promises] and [nacks].  Acceptor/learner remain Idle / empty. *)
...
let%expect_test "quorum/phase1: majority nacks → MajorityNacks" =
  let nacks = [nack_of 1 "foo" blank_promise; nack_of 1 "foo" blank_promise] in
...
let accepting_state ?(acks = []) ?(nacks = []) seq value : NS.role_state =
  let rs = NS.init_state () in
  let a = assertion seq value in
  let pa = NS.ProposerAccepting {assertion= a; acks; nacks_received= nacks} in
...
(** 3: Proposal permissibility
     [notes]
     1. we just use pid values in the examples below because we just treat it as self receive.
  *)
