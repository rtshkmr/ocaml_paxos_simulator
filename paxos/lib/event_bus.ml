  ...
  *)
  ...
  let unsubscribe t handle =
    ...
    | Some ts ->
      ...
      if List.is_empty ts.subs && ts.published = 0 && ts.queued = 0 && ts.delivered = 0
  ...
  let publish t ~topic payload =
    ...
    (match t.logger with
     | Some f -> ignore (f topic payload)  (* logger side-effect only; caller's logger can persist it *)
     | None -> ());
    match ts.subs with
    | [] -> ()
    | callbacks ->
  ...
  let drain t =
    while not (Queue.is_empty t.queue) do
      ...
      (match Hashtbl.find t.topics topic with
       | None -> ()
       | Some ts -> ts.queued <- Int.max 0 (ts.queued - 1));
  ...
  let stats t =
    ...
    |> List.map ~f:(fun (topic, ts) -> (topic, (List.length ts.subs, ts.published, ts.delivered, ts.queued)))
end
