...
module Event_bus : S = struct
  (** Polymorphic in-process pub/sub message broker for deterministic
      simulation.

      {b Design:}
      - Synchronous publish/subscribe for v0 (no concurrency yet for this
        version)
  ...
  type 'a enqueuable_thunk = (Types.topic * Types.node_id option) * 'a
  (** A thunk that can be queued such that it will either be published as a
      broadcast or as a publish to a single node *)
  ...
  (** Returns the [topic_state] for [topic] if exists else initialises one for
      that topic and returns it. This allows lazy creation of topic entries. *)
  ...
  let publish_broadcast ({ id; logger; payload_to_string; _ } as t) ~topic
      payload =
    let ({ published; subs; delivered; _ } as ts) =
      ensure_topic_state t topic
    in
  ...
  let publish_unicast { topics; id; payload_to_string; logger; _ } ~topic
      ~node_id payload =
