...
(** Core, shared primitive types for the whole system.
    ...
    Intent:
    - Provide canonical definitions for node identifiers and proposal
      identifiers.

    All of these are comparable and hashable, they may be used as keys for
    containers like map/set
...
module Types : sig
  ...
  (** When driving consensus, a node would need to assert their own value first.
      ...
      NOTE [semantics]: this is named [assertion] in the context of the paxos
      protocol in that:
      - peer nodes assert on what they think the value (the state that we desire
        to seek consensus on) will be
      - the word "assertion" is unrelated to the programming construct
        "assertion" *)

  type 'a paxos_promise = 'a paxos_assertion_state option
  [@@deriving sexp, compare, equal, yojson]
  (** NOTE [semantics]: this is named [promise] in the context of the paxos
      protocol in that:
      - acceptors receive promises on what the value (the state that we desire
        to seek consensus on) will be
...
(** Modules that satisfy this interface define a specific spec type which can be
    used to generate structs of that module.*)
