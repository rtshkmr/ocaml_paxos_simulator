...
(** Discrete-time deterministic simulator for Paxos protocol testing.
    ...
    {b Partitioning:} Nodes belong to partitions (network islands). Each
    partition has its own bus. Nodes in different partitions cannot communicate
...
module Simulator : Runtime.S = struct
  ...
  let dispatch_heartbeat
      ({ registries = { partition_registry; _ }; clock; _ } as sim) =
    let msg =
  ...
  let is_runnable ({ settings = { max_ticks; _ }; _ } as sim) =
    ...
    | Some limit -> sim |> current_time <= limit
    | _ -> true
  ...
  let move_node_to_partition_exn
      ({
         ...
         _;
       } as sim) ~(dest : partition_id) node_id =
  ...
  let make_node_change_partitions
      ({ logger; registries = { node_alias_registry; _ }; _ } as sim) ~args
      alias =
  ...
  let control_node_state_change sim ~time alias action_type =
    ...
    let factory =
      ...
      | `Idle -> Message.make_sim_control_idle_node
      | `Inactive -> Message.make_sim_control_inactive_node
      | `Active -> Message.make_sim_control_activate_node
    in
  ...
  let seed_node_exn
      ({
         registries =
           { partition_registry; node_registry; node_alias_registry; _ };
         _;
       } as sim :
        t) (node : N.t) =
  ...
  let hydrate_proposal_event sim
    ...
    | None, _ | _, None -> failwith "Malformed proposal event spec"
  ...
  let hydrate_metric_event
      ({ registries = { partition_registry; _ }; _ } as sim)
    ...
    | _ -> failwith ("Unknown metric data: " ^ Option.value ~default:"" data)
  ...
  let make_control_action sim ({ target; data; time; args; _ } : Sim_event.spec)
    ...
    | _ -> failwith "Malformed control event spec"
  ...
  let of_spec ?(override_log_level = Log_level.Debug) { max_ticks; log_level } :
      t =
  ...
  let handle_slash_command sim cmd_s =
    ...
    | [ "help" ] -> Logger.slash_cmd_help sim.logger
    | [ "inspect"; "sim"; "state" ] ->
    ...
    | [ "inspect"; "node"; "state"; alias ] -> (
        ...
        | None -> display_node_not_found alias
        | Some node ->
    ...
    | [ "inspect"; "bus"; "stats"; bus_id ] -> (
        ...
        if Option.is_none parsed_bus_id then
          ...
          | None -> display_bus_not_found parsed
          | Some bus -> Event_bus.display_stats bus)
    | fallthrough ->
