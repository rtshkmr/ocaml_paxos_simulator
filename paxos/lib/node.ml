...
module Make_node (V : Value.S) (Bus : Event_bus.S) :
  ...
  (** v0: for simple paxos, we shall just keep it to proposal id, can expand to
      (proposal_id, node_id) for multi-paxos *)
  ...
  (** Polymorphic transition function for role state. This is a classic
      ...
      Learning NOTE: 1. Importance of locally abstract types for type safety
      - [(type a)] introduces a locally abstract type [a] scoped within the
        function, tied by GADT patterns to a specific substate type
        ([proposer_state], [acceptor_state], or [learner_state]).
      - Locally abstract types enable type-safe polymorphic dispatch: each
        constructor of the GADT carries different precise type information for
        ['a].
      - The function can only accept or return values consistent with ['a] as
        determined by the GADT constructor.
      - This is what makes GADT-based functions type-safe and flexible without
        unsafe casts or polymorphic variants. *)
  ...
  let handle_permission_request node msg =
    ...
    let reply =
      ...
      | true ->
          ...
          (* state transition *)
          State.Accepting
            { promised = Some assertion; accepted = last_accepted }
  ...
  let activate time node =
    ...
    | Error _e -> node.state <- State.idle_of node.state
    | Ok restored -> (
        node.state <- restored;
        match Storage.persist_snapshot !(node.storage) time restored with
        | Ok updated -> node.storage := updated
        | Error e ->
            log_decision node
              ~msg:
  ...
  (** TODO [extension v2]: aborting will likely involve retrying mechanisms and
      such*)
  let abort node =
  ...
  let handle_permission_granted node msg =
    ...
    | _ -> noop_ignore node "not waiting for promises"
  ...
  let handle_suggestion node msg =
    ...
    let reply =
      ...
      | true ->
          State.Accepting
            { promised = Some assertion; accepted = Some assertion }
          |> transition_role_state node time State.Acceptor;
  ...
  let handle_accepted node msg =
    ...
    let { meta = { id; timestamp; _ }; from; _ } : V.t Message.accepted_msg =
      msg
    in
    match node.state.proposer with
    | State.ProposerAccepting pa -> (
        ...
        | _ -> ())
    | _ -> noop_ignore node "not waiting for accepted msgs"
  ...
  let handle_nack node msg =
    ...
    let { meta = { topic; id = rcvd_msg_id; timestamp }; _ } :
  ...
  let get_handler_for_topic node topic : V.t Bus.bus_registrable_callback =
    ...
    match topic with
    | Types.Coordination -> node |> handle_coordination
    | Types.Simulation_control -> node |> handle_simulation_control
    | Types.Time -> node |> handle_time
    | _ -> failwith "Unsupported topic for message passing"

  let register_node_with_bus bus
      ({ config = { topics; _ }; id = node_id; alias = node_alias; subs; _ } as
       node) =
    let log_msg =
