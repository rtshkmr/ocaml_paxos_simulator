...
module Message = struct
  ...
  let meta_of = function
    | Coordination msg -> msg |> meta_of_coordination
    | Control msg -> msg |> meta_of_control
    | Time msg -> msg |> meta_of_time
  ...
  let coordination_proposal = function
    ...
    | Accepted { assertion = { proposal; _ }; _ } ->
        Some proposal
    | PermissionGranted { assertion = { proposal; _ }; _ } -> Some proposal
    | Decided { decided_assertion = { proposal; _ }; _ } -> Some proposal
    | Nack { rejected_assertion = { proposal; _ }; _ } -> Some proposal

  let proposal_id_of = function
    | Coordination msg -> msg |> coordination_proposal
    | _ -> None
