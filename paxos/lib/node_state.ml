...
module type S = sig
  ...
  (** In Paxos, an [assertion] is a proposed value paired with a proposal ID. It
      represents what a proposer believes the chosen value should be.
      - the word "assertion" is unrelated to the programming construct
        "assertion" *)
...
module Make_node_state (V : Value.S) = struct
  ...
  type proposer_state =
    | ProposerInactive
        (** this state encodes it's unavailability. When a node is inactivated,
            it is effectively killed -- it must look to its storage to resume
            partitipation thereafter *)
    | Idle
        (** An node may be idle to indicate that it can be a valid participant
            (by initiating a proposal)*)
    | Preparing of assertion
        (** A node that is initiating a proposal will be in the preparing state.
            It will be ready to .*)
    | WaitingForPromises of waiting_for_promise_state
        (** A node that is gathering responses to their proposal and is waiting
            to reach a quorum of responses.*)
    | ProposerAccepting of proposer_accepting_state
        (** A node that has suggested *)
    | Decided of V.t
        (** A node that has decided on the value that consensus has been
            achieved for*)
  [@@deriving sexp, yojson]
  ...
  let is_inactive { acceptor; proposer; _ } =
    ...
    | AcceptorInactive, _ | _, ProposerInactive -> true
    | _ -> false
  ...
  let current_promise ({ acceptor; _ } : role_state) =
    ...
    | Idle ->
        ...
        (*      alias ) ; *)
        None
    | Accepting { promised; _ } -> promised
    | _ -> assert false

  let last_accepted_promise ({ acceptor; _ } : role_state) =
    match acceptor with
    | Idle -> None
    | Accepting { accepted; _ } -> accepted
    | _ -> assert false

  (* TODO Verify this quorum determination below for the WaitingForPromises is correct *)

  (** Consider the highest proposal_id from all promises received. case 1: it's
      None, use proposers own value case 2: it's Some, pick associated value for
      highest proposal id *)
  ...
  type acceptor_record_spec = {
    promised : V.t Types.promise_spec;
    accepted : V.t Types.promise_spec;
  }
  [@@deriving sexp, yojson]
  ...
  (* ==== specs and of_specs *)
  type nack_spec = {
    rejected_assertion : V.t Types.assertion_spec;
    hint : V.t Types.promise_spec;
  }
  [@@deriving sexp, yojson]
  ...
  let proposer_state_of_spec = function
    | ProposerInactive_spec -> ProposerInactive
    | Idle_spec -> Idle
    ...
    | Decided_spec v_spec -> v_spec |> V.of_spec |> Decided
  ...
  let acceptor_state_of_spec = function
    | AcceptorInactive_spec -> AcceptorInactive
    | Idle_spec -> Idle
    | Accepting_spec spec -> spec |> acceptor_record_of_spec |> Accepting
